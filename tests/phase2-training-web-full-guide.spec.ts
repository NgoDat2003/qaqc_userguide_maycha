...
for (const viewport of [
  ...
  test(`${viewport.name}: full Phase 2 Training guide`, async ({ page }) => {
    ...
    expect(bodyText).toContain('Word chưa đồng bộ Phase 2');

    const wordButton = page.locator('button.download-button');
...
test('legacy Word URLs return 404 and dist contains no DOCX', async ({ request }) => {
  const legacyWordFiles = [
    'huong-dan-qaqc.docx',
    'huong-dan-su-dung-he-thong-qaqc-maycha-01-06-2026.docx',
    'huong-dan-su-dung-he-thong-qaqc-uat-27-05-2026.docx',
  ];

  for (const fileName of legacyWordFiles) {
    const response = await request.get(`${BASE_URL}/downloads/${fileName}`);
    expect(response.status()).toBe(404);
  }

  const distFiles = readdirSync(join(process.cwd(), 'dist'), { recursive: true });
  expect(distFiles.filter((file) => file.toString().toLowerCase().endsWith('.docx'))).toEqual([]);
});
test('desktop phase parents behave as a single-open accordion', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.goto(`${BASE_URL}/#phase-2`, { waitUntil: 'domcontentloaded' });
  ...
  await expect(sidebar.locator('#desktop-phase-1-chapters')).toHaveCount(0);
...
test('mobile drawer keeps parent toggles open and restores deep-link expansion', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto(`${BASE_URL}/#phase-2`, { waitUntil: 'domcontentloaded' });
  await page.locator('.mobile-toc-button').click();
  ...
  await expect(phase1Toggle).toHaveAttribute('aria-expanded', 'false');
  await expect(phase2Toggle).toHaveAttribute('aria-expanded', 'true');
  await expect(drawer.locator('#mobile-phase-1-chapters')).toHaveCount(0);
